...
`)
	...
	testCases := []struct {
		...
		{
			name:    "NewTranslator: 语言文件名非法，Parse 失败",
			langDir: tmpDir,
			key:     "hello",
			lang:    "en-US",
			prepare: func(dir string) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			trans, err := NewTranslator(tc.langDir)
			if tc.newTransErr {
				require.Error(t, err)
				return
			}
...
func TestTranslator_MustTranslate(t *testing.T) {
	tmpDir := t.TempDir()
	writeTestLangFile(t, tmpDir, "en-US", `
- id: hello
  translation: "Hello"
...
`)
	...
	trans, err := NewTranslator(tmpDir)
	require.NoError(t, err)
	ctx := context.Background()
